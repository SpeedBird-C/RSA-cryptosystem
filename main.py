...
class RSA:
    ...
    def decrypt(self, m, n, d, m_last_size):
        ...
        return res
    def decrypt1(self, m, n, d):
        res = bytes()
        bs_pl = math.ceil(math.floor(math.log2(n)) / 8) - 1
        bs_enc = math.ceil(math.floor(math.log2(n)) / 8)
        for i in range(0, len(m), bs_enc):
            t = int.from_bytes(m[i:i + bs_enc], 'big')
            c = (pari(t).Mod(pari(n)) ** pari(d)).lift()
            res += int(c).to_bytes(bs_pl, 'big')
        return res

    def sign(self, m, n, d):
        h = SHA256.new() #вычисляем хэш образ
        h.update(m)
        r = h.digest()
        return self.encrypt(r, n, d) # зашифр хэш

    def verify(self, s, m, n, e):
        r = self.decrypt1(s, n, e) #предполгагаемый хэш-образ путём рашифровки
        h = SHA256.new() #вычисл хэш-образ
        h.update(m)
        return int.from_bytes(r, 'big') == int.from_bytes(h.digest(), 'big') #сравниваем
...
def P1sign(read_name, save_name):
    ...
    rsa = RSA(RSA_size)
    # n, e, d = rsa.genKeys()
    s = rsa.sign(data, n, d) #зашифр хэш
